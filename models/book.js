...
class Book {
  ...
  static async update(isbn, data) {
    ...
    queryString += ` WHERE isbn=($${count + 1}) 
        RETURNING isbn,
                  amazon_url,
                  author,
                  language,
    ...
    const fieldValues = Object.keys(data).map((key) => data[key]);
    fieldValues.push(isbn);

    const result = await db.query(queryString, fieldValues);
